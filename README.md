...
| **교통사고 다발구역** | 2024년 기준 서울 3,743개 지점 원형 마커 · 호버 팝업 · 토글 |
| **오늘 교통량 차트** | 실시간 모드 한정 · 24시간 시간별 막대그래프 |
| **구별 혼잡도 랭킹** | 5,800+ 링크 속도를 25개 자치구 단위로 집계 · 혼잡/원활 TOP 5 패널 |
| **자치구 라벨** | 서울특별시 · 25개 자치구 이름 항상 표시 · 데이터 선택 시 혼잡도 색상 + 평균 속도 반영 |
| **막히는 도로 TOP 5** | 도로명 단위 평균 속도 집계 · 클릭 시 해당 도로로 카메라 이동 + 고정 팝업 |
| **도로 호버 팝업** | 도로 링크에 마우스 올리면 도로명 · 현재 속도 · 혼잡 상태 즉시 표시 |
...
- **Data** — 서울시 TOPIS 실시간 교통 API, TOPIS 도로 통계 API, 공공데이터포털 교통사고 다발구역
- **i18n** — next-intl
...
│  [출근 시간]    3D 서울 씨티맵                       │
│  [낮 시간  ]    (도로 색상 = 속도)                   │
│  [퇴근 시간]    (구 이름 라벨 = 혼잡도 색상)         │
│  [사고 다발구역](빨간 원 = 사고 다발구역)            │
│                                                      │
│  [◎]  [막히는 도로 TOP5] [구별 혼잡도 랭킹]          │
│                          [속도 범례 + 사고 기준]      │
└─────────────────────────────────────────────────────┘
```
